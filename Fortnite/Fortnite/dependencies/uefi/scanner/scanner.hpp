...
namespace scanner
{
	/*inline uintptr_t pattern_scan( uint32_t start, uint32_t size, const char* pattern ) {
		...
		for ( int i = 0; i < size; i += chunk_size )
			vm.get( )->read_large_array( start + i, buf + i, chunk_size );
	...
	{
		IMAGE_DOS_HEADER DOSHeader = vm.get( )->read<IMAGE_DOS_HEADER>( base );
		IMAGE_NT_HEADERS NTHeaders = vm.get( )->read<IMAGE_NT_HEADERS>( base + DOSHeader.e_lfanew );
